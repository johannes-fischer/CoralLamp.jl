...
begin
	...
	ϕ1 $(@bind ϕ1 Slider(0:0.01:1.5, default=0.5))
	
	d $(@bind d Slider(r1:0.01:tip, default=tip/4))
	"""
end
...
function draw_head_piece(R1, R2, ϕ1, d)
	
	arc(-side, 0, r1, pi/2, -pi/2)
	carc(-1.5r1, -1.5r1, r1/2, pi/2, 0)
	
	carc(-(r1 + R2), -(tip - d), R2, 0, -ϕ1)
	arc(0, -tip, R1, -pi-ϕ1, ϕ1)
	carc(r1 + R2, -(tip - d), R2, -pi + ϕ1, -pi)
...
@draw let
	...
	R2_1 = 1 / (1 - cos(ϕ1)) * (R1 * cos(ϕ1) - r1)
	d_1 = (R1 + R2_1) * sin(ϕ1)
	
	@layer begin
		translate(-side, 0)
		draw_head_piece(R1, R2_1, ϕ1, d_1)
	end
	
	R2_2 = (d^2 + r1^2 - R1^2) / (2(R1 - r1))
	ϕ1_2 = asin(d / (R1 + R2_2))
	@assert ϕ1_2 ≈ acos((r1+R2_2)/(R1+R2_2))
	@layer begin
		translate(side, 0)
		draw_head_piece(R1, R2_2, ϕ1_2, d)
	end
end 800 500
